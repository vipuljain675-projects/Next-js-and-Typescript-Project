...
const Navbar = () => {
  ...
  // Get User Initial (e.g., 'V' from Vipul)
  const userInitial = auth?.user?.firstName ? auth.user.firstName.charAt(0).toUpperCase() : "";

  return (
    <header className="fixed-top bg-white border-bottom" style={{ zIndex: 1030 }}>
      <div className="container-fluid px-4 px-md-5">
        <div className="d-flex justify-content-between align-items-center" style={{ height: "80px" }}>
          ...
          <Link href="/" className="text-decoration-none d-flex align-items-center">
            <svg width="32" height="32" fill="#FF385C" viewBox="0 0 32 32">
            ...
            <span className="ms-2 fw-bold fs-4 d-none d-sm-inline" style={{ color: "#FF385C" }}>airbnb</span>
          </Link>
          ...
          <div className="d-flex align-items-center gap-3">
            {auth?.isLoggedIn ? (
               <div className="d-none d-md-flex align-items-center gap-3">
                  <Link href="/bookings" className="text-dark fw-semibold small text-decoration-none">Trips</Link>
                  <Link href="/wishlist" className="text-dark fw-semibold small text-decoration-none">Saved</Link>
                  
                  {/* Messages Link with Badge */}
                  <Link 
                    href="/messages" 
                    className="text-dark fw-semibold small text-decoration-none position-relative"
                  >
                  ...
                  <Link href="/host" className="text-dark fw-semibold small text-decoration-none">Switch to Host</Link>
               </div>
            ) : (
            ...
            <div className="position-relative" onClick={(e) => e.stopPropagation()}>
              ...
              {/* 4. Dropdown Menu */}
              {showUserMenu && (
                <div className="position-absolute end-0 mt-2 bg-white rounded-4 shadow-lg border py-2 text-start" style={{ width: "240px", zIndex: 1050 }}>
                  {auth?.isLoggedIn ? (
                    <>
                      <div className="px-4 py-2 fw-bold small text-secondary">Hi, {auth.user?.firstName}</div>
                      <Link href="/messages" className="d-block px-4 py-2 text-decoration-none text-dark hover-bg-light">
                      ...
                      <Link href="/bookings" className="d-block px-4 py-2 text-decoration-none text-dark hover-bg-light">My Trips</Link>
                      <Link href="/wishlist" className="d-block px-4 py-2 text-decoration-none text-dark hover-bg-light">Saved Homes</Link>
                      <hr className="my-2" />
                      <Link href="/host/my-listings" className="d-block px-4 py-2 text-decoration-none text-dark hover-bg-light">Manage Listings</Link>
                      <Link href="/host/manage-bookings" className="d-block px-4 py-2 text-decoration-none text-dark hover-bg-light">Handle Requests</Link>
                      <hr className="my-2" />
                      <button onClick={auth.logout} className="d-block w-100 text-start px-4 py-2 border-0 bg-transparent text-danger fw-bold hover-bg-light">Log out</button>
                    </>
                  ) : (
                    <>
                      <Link href="/login" className="d-block px-4 py-2 text-decoration-none text-dark hover-bg-light fw-bold">Log in</Link>
                      <Link href="/signup" className="d-block px-4 py-2 text-decoration-none text-dark hover-bg-light">Sign up</Link>
                      <hr className="my-2" />
                      <Link href="/host" className="d-block px-4 py-2 text-decoration-none text-dark hover-bg-light">Airbnb your home</Link>
                      <Link href="/help" className="d-block px-4 py-2 text-decoration-none text-dark hover-bg-light">Help Center</Link>
                    </>
                  )}
                </div>
              )}
